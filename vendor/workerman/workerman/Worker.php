...
class Worker
{
    ...
    protected static function displayUI()
    {
        ...
        if (static::$_OS !== OS_TYPE_LINUX) {
            static::safeEcho("------------------------ AlexMVC -----------------------------\r\n");
            static::safeEcho('Based Workerman :'. static::VERSION. "          PHP version:". PHP_VERSION. "\r\n");
            static::safeEcho('AlexMVC version :1.0.2'."          os: ".      $_SERVER['OS']. "\r\n");
        ...
        //show version
        $line_version = 'Based Workerman :' . static::VERSION . str_pad('PHP version:', 22, ' ', STR_PAD_LEFT) . PHP_VERSION . PHP_EOL;
        $line_versiona = 'AlexMVC version :1.0.2' .str_pad(' type :'.$_SERVER['HOSTTYPE'], 23, ' ', STR_PAD_LEFT) . PHP_EOL;
        !defined('LINE_VERSIOIN_LENGTH') && define('LINE_VERSIOIN_LENGTH', strlen($line_version));
        $total_length = static::getSingleLineTotalLength();
        $line_one = '<n>' . str_pad('<w> AlexMVC </w>', $total_length + strlen('<w></w>'), '-', STR_PAD_BOTH) . '</n>'. PHP_EOL;
        $line_two = str_pad('<w> WORKERS </w>' , $total_length  + strlen('<w></w>'), '-', STR_PAD_BOTH) . PHP_EOL;
        static::safeEcho($line_one . $line_version . $line_versiona . $line_two);
